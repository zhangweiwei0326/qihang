...
class FitmentAction extends PublicAction {
    public function index() {
        ...
        if($_GET[id] == "" || $_GET[id] == null){
            ...
            $list = $Dao->table('my_fitment fitment,my_category category')
                    ->where('fitment.cate_id = category.id and fitment.ifShow = 1')
            ...
            $list = $Dao->Table('my_fitment fitment,my_category category')
                    ->where('fitment.cate_id = category.id and category.id =' . $_GET[id] .' and fitment.ifShow = 1')
                    ->field('fitment.id as id,fitment.title,fitment.summary,fitment.content,fitment.add_date,fitment.click_count,category.name as typeName')
					->limit($p->firstRow.','.$p->listRows)
					->order('fitment.add_date desc')
                    ->select();	
    ...
    public function cateShow() {
        header("Content-Type:text/html; charset=utf-8");
        $Dao = M("category");  
        $list = $Dao->where('type = "fitment" and is_show = 1')
					->select();
    ...
    public function showDetail() {
        ...
        $sql = "update my_fitment set click_count = click_count + 1 where id = '$_GET[id]';";
        ...
        $this->pubHtml();
        $this->cateShow();
        $this->showComment($_GET[id],'fitment');
		$User = A("Index"); // 实例化UserAction控制器对象
        $hotlist = $User->hotShow(); // 调用User模块的importUser操作方法
		$this->assign("hotlist", $hotlist);
        $this->display();
	...
	public function toSearch(){
		...
		$p = new Page($count,5); 
        $list = $Dao->Table('my_fitment fitment,my_category category')
                    ->where('fitment.cate_id = category.id and fitment.title like  "%'.$_POST['keyName'].'%" and fitment.ifShow = 1')
                    ->field('fitment.id as id,fitment.title,fitment.summary,fitment.content,fitment.add_date,fitment.click_count,category.name as typeName')
					->limit($p->firstRow.','.$p->listRows)
					->order('fitment.add_date desc')
                    ->select();	
